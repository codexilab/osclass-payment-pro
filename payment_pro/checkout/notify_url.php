<?php
    ...
    if (strcmp($res, 'VERIFIED') == 0) {
        if ($_REQUEST['payment_status'] == 'Completed') {
            ...
            $conn = getConnection();
            $payment = $conn->osc_dbFetchResult("SELECT * FROM %st_shop_paypal_log WHERE s_code = '%s'", DB_TABLE_PREFIX, Params::getParam('return_code'));
            if (!isset($payment['pk_i_id'])) {
                // Payment is not processed yet
                $conn->osc_dbExec("INSERT INTO %st_shop_paypal_log (s_concept, dt_date, s_code, f_amount, s_currency_code, s_email, fk_i_transaction_id) VALUES ('%s', '%s', '%s', %f, '%s', '%s', %d)", DB_TABLE_PREFIX, Params::getParam('item_name'), date('Y-m-d H:i:s'), Params::getParam('return_code'), Params::getParam('amount'), Params::getParam('currency_code'), Params::getParam('payer_email'), Params::getParam('item_number'));
